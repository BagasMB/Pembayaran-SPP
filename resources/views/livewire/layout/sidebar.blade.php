...
<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
    ...
    <ul class="menu-inner py-1 pb-5">
        ...
        @foreach ($angkatan as $list)
            @php
                $studentM = new \App\Models\Student();
            @endphp
            <li class="menu-item {{ request()->is('student/class/' . $list['tahun_masuk'] . '/*') ? 'active open' : '' }}"
                style="">
                <a href="javascript:void(0);" class="menu-link menu-toggle">
                    <i class="menu-icon tf-icons bx bx-detail"></i>
                    <div data-i18n="Form Elements">{{ $studentM->angkatan($list['tahun_masuk']) }}</div>
                </a>
                <ul class="menu-sub">
                    @foreach ($class as $cas)
                        <li
                            class="menu-item {{ request()->is('student/class/' . $list['tahun_masuk'] . '/' . $cas->id) ? 'active' : '' }}">
                            <a href="/student/class/{{ $list['tahun_masuk'] }}/{{ $cas->id }}" class="menu-link">
                                <div data-i18n="Basic Inputs">{{ $cas->name_class }}</div>
                            </a>
                        </li>
                    @endforeach
                </ul>
        ...
        @can('masterdata')
            ...
            @php
                $isUserSectionActive = request()->is('student') || request()->is('teacher') || request()->is('user');
            @endphp

            <li class="menu-item {{ $isUserSectionActive ? 'active open' : '' }}">
                <a href="javascript:void(0);" class="menu-link menu-toggle">
                ...
                <ul class="menu-sub">
                    <li
                        class="menu-item {{ request()->is('student') || request()->is('tagihan-student/*') ? 'active' : '' }}">
                    ...
                    @can('user')
                        <li class="menu-item {{ request()->is('user') ? 'active' : '' }}">
                            <a href="{{ route('user.index') }}" class="menu-link">
                                <div data-i18n="Basic Inputs">User Pengguna</div>
                            </a>
                        </li>
                    @endcan
                </ul>
            ...
            <li class="menu-item {{ request()->is('major') ? 'active' : '' }}">
                <a href="/major" class="menu-link">
                    <i class="menu-icon tf-icons fa-solid fa-person-chalkboard"></i>
                    <div data-i18n="major">Majors</div>
                </a>
            </li>

            <li class="menu-item {{ request()->is('classroom') ? 'active' : '' }}">
                <a href="/classroom" class="menu-link">
                    <i class="menu-icon tf-icons fa-solid fa-school"></i>
